// URL Web App daripada Google Apps Script (Ganti dengan URL anda)
const GAS_URL = "URL_WEB_APP_GAS_ANDA";

// 1. PENDAFTARAN SERVICE WORKER (PWA)
...
// 2. DATA SILIBUS (DIAMBIL DARI SILIBUS.HJSON)
const silibusData = {
    "1": [
    ...
    "3": [
        ...
        { nama: "Al-Infithor", ayat: 19, ms: 587 } [cite: 4]
    ],
    "4": [
        { nama: "Al-Buruj", ayat: 22, ms: 590 }, { nama: "Al-Insyiqaq", ayat: 25, ms: 589 },
        { nama: "At-Takwir", ayat: 29, ms: 586 }, { nama: "Abasa", ayat: 42, ms: 585 } [cite: 5]
    ],
    "5": [
        { nama: "Al-Muthoffifin", ayat: 36, ms: 587 }, { nama: "Al-Fajr", ayat: 30, ms: 593 },
        { nama: "An-Nazi'aat", ayat: 46, ms: 583 }, { nama: "An-Naba'", ayat: 40, ms: 582 } [cite: 5]
    ],
    "6": [
        ...
        { nama: "Al-Jumu'ah", ayat: 11, ms: 553 }, { nama: "Al-Kahfi", ayat: 110, ms: 293 } [cite: 6]
    ]
};
...
function renderSilibus() {
    ...
    grid.innerHTML = data.map(s => `
        <div class="surah-card glass-card">
            ...
            <button class="btn-check" onclick="simpanRekod('${s.nama}', ${tahap})">Selesai Tasmik</button>
        </div>
    `).join('');
}

// 4. LOGIK PENYIMPANAN (SMART OFFLINE)
function simpanRekod(surah, tahap) {
    const nama = document.getElementById('nama').value;
    const umur = document.getElementById('umur').value;

    if (!nama || !umur) {
        alert("Sila masukkan Nama dan Umur peserta!");
        return;
    }
    ...
    // Simpan ke LocalStorage (Cache Data)
    let localData = JSON.parse(localStorage.getItem('tasmik_records')) || [];
    localData.push(dataTasmik);
    localStorage.setItem('tasmik_records', JSON.stringify(localData));

    // Cuba hantar ke GAS jika ada internet
    syncKeGAS();
    
    alert(`Rekod ${surah} untuk ${nama} telah disimpan secara offline.`);
}

// 5. SYNC KE GOOGLE APPS SCRIPT
async function syncKeGAS() {
    ...
    try {
        ...
        // Jika berjaya, kosongkan cache offline
        localStorage.removeItem('tasmik_records');
        console.log("Data Berjaya Disinkronisasi ke Google Sheets");
    } catch (err) {
        console.log("Sync Gagal, Data Kekal di Telefon", err);
    }
}
